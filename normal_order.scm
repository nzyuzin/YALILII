...
(define (and-first-term exp) (cadr exp))

(define (and-rest-terms exp) (cddr exp))

(define (eval-and exp env)
  (if (null? (and-rest-terms exp))
    (eval (and-first-term exp) env)
    (if (true? (eval (and-first-term exp) env))
      (eval-and (cdr exp) env)
      false)))

(define (or? exp) (tagged-list? exp 'or))

(define (or-first-term exp) (cadr exp))

(define (or-rest-terms exp) (cddr exp))

(define (eval-or exp env)
  (if (true? (eval (or-first-term exp) env))
    (eval (or-first-term exp) env)
    (if (null? (or-rest-terms exp))
      false
      (eval-or (cdr exp) env))))
...
;; EXERCISE 4.5 SPECIAL COND
;================================================================

(define (special-clause? clause)
  (and 
    (not (null? (cond-predicate clause)))
    (equal? (cadr clause) '=>) 
    (not (null? (caddr clause)))
    (null? (cdddr clause))))
...
(define (fetch-defines expr def-names def-values rest-of-expr)
  (if (null? expr)
...
;; CHANGED AS PART OF EXERCISE 4.16
;; ==========================================================
      ...
      (cond ((null? vars)
            ...
            ((eq? var (car vars))
             (if (eq? (car vals) '*unassigned*)
               (error "Unassigned variable" (car vals))
               (car vals)))
